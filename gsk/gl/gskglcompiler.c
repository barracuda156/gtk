...
#define SHADER_VERSION_GL2_LEGACY "120"
                     ...
                     gboolean     debug_shaders)
{
  ...
  if (gdk_gl_context_get_use_es (context))
  ...
  else if (gdk_gl_context_is_legacy (context))
    {
      ...
      /* Make context current so glGetString() is valid */
      gsk_gl_command_queue_make_current (driver->shared_command_queue);

      gdk_gl_context_get_version (context, &maj, &min);

      if (maj >= 3)
        {
          /* Verify the driver actually supports GLSL 1.30.
           * Some drivers report GL 3.x but GLSL support tops out at 1.20
           * or lower (common on older macOS and some X11 drivers).
           * We also cap at 130 unconditionally — legacy shaders use
           * attribute/varying and never need anything higher.
           */
          const char *glsl_str = (const char *) glGetString (GL_SHADING_LANGUAGE_VERSION);
          int glsl_maj = 1, glsl_min = 0;

          if (glsl_str)
            sscanf (glsl_str, "%d.%d", &glsl_maj, &glsl_min);

          if (glsl_maj > 1 || (glsl_maj == 1 && glsl_min >= 30))
            self->glsl_version = SHADER_VERSION_GL3_LEGACY;  /* "130" */
          else
            self->glsl_version = SHADER_VERSION_GL2_LEGACY;  /* "110" */
        }
      else
        self->glsl_version = SHADER_VERSION_GL2_LEGACY;  /* "110" */
