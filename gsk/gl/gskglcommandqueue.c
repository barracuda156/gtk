                          ...
                          GskGLUniformState *uniforms)
{
  ...
  /* Only trust GL_ARB_sampler_objects if the driver also supports GLSL 1.30,
   * since genuine GL 3.3 requires both. Drivers that advertise the extension
   * but can't do GLSL 1.30 don't actually export glGenSamplers. */
  {
    ...
    gboolean has_glsl_130 = (glsl_maj > 1 || (glsl_maj == 1 && glsl_min >= 30));
    self->has_samplers = has_glsl_130 && epoxy_has_gl_extension ("GL_ARB_sampler_objects");
  }
                         ...
                         GLint              gl_swizzle[4])
{
  ...
  if ((flags & required_flags) == required_flags)
    {
      gdk_memory_format_gl_format (data_format,
                                   ...
                                   gl_type,
                                   &gdk_sw);
      gdk_swizzle_to_gl (gdk_sw, gl_swizzle);
  ...
  {
    GdkSwizzle alt_swizzle;
    if (gdk_memory_format_get_rgba_format (data_format, &alt_format, &alt_swizzle))
      {
        ...
        if ((flags & required_flags) == required_flags)
          {
            *out_can_mipmap = (flags & GDK_GL_FORMAT_RENDERABLE) ? TRUE : FALSE;
            ...
            if (self->can_swizzle)
              {
                /* Upload as alt_format, but swizzle to display as data_format */
