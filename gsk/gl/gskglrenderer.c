 ...
 * Since: 4.2
 */
struct _GskGLLegacyRendererClass
{
  GskRendererClass parent_class;
};

struct _GskGLLegacyRenderer
{
...
static gboolean
gsk_gl_legacy_renderer_dmabuf_downloader_supports (GdkDmabufDownloader  *downloader,
...
static gboolean
gsk_gl_legacy_renderer_dmabuf_downloader_download (GdkDmabufDownloader   *downloader_,
                                             GdkDmabufTexture      *texture,
                                             guchar                *data,
...
static void
gsk_gl_legacy_renderer_dmabuf_downloader_close (GdkDmabufDownloader *downloader)
{
  gsk_renderer_unrealize (GSK_RENDERER (downloader));
}

static void
gsk_gl_legacy_renderer_dmabuf_downloader_init (GdkDmabufDownloaderInterface *iface)
{
  iface->close = gsk_gl_legacy_renderer_dmabuf_downloader_close;
  iface->download = gsk_gl_legacy_renderer_dmabuf_downloader_download;
}

G_DEFINE_TYPE_EXTENDED (GskGLLegacyRenderer, gsk_gl_legacy_renderer, GSK_TYPE_RENDERER, 0,
                        G_IMPLEMENT_INTERFACE (GDK_TYPE_DMABUF_DOWNLOADER,
                                               gsk_gl_legacy_renderer_dmabuf_downloader_init))

/**
 * gsk_gl_legacy_renderer_new:
 *
 ...
 */
GskRenderer *
gsk_gl_legacy_renderer_new (void)
{
  return g_object_new (GSK_TYPE_GL_LEGACY_RENDERER, NULL);
}

static gboolean
gsk_gl_legacy_renderer_realize (GskRenderer  *renderer,
                         GdkDisplay   *display,
                         GdkSurface   *surface,
                         gboolean      attach,
                         GError      **error)
{
  ...
  GskGLLegacyRenderer *self = (GskGLLegacyRenderer *)renderer;
...
failure:
  ...
  gdk_profiler_end_mark (start_time, "realize GskGLLegacyRenderer", NULL);
...
static void
gsk_gl_legacy_renderer_unrealize (GskRenderer *renderer)
{
  GskGLLegacyRenderer *self = (GskGLLegacyRenderer *)renderer;

  g_assert (GSK_IS_GL_LEGACY_RENDERER (renderer));
                   ...
                   GdkGLContext *context)
{
   ...
   */
  damage = gdk_draw_context_get_render_region (GDK_DRAW_CONTEXT (context));
...
static void
gsk_gl_legacy_renderer_render (GskRenderer   *renderer,
                        GskRenderNode        *root,
...
static GdkTexture *
gsk_gl_legacy_renderer_render_texture (GskRenderer    *renderer,
                                GskRenderNode         *root,
      ...
      for (y = 0; y < height; y += max_size)
        {
          for (x = 0; x < width; x += max_size)
            {
              texture = gsk_gl_legacy_renderer_render_texture (renderer, root,
...
static void
gsk_gl_legacy_renderer_dispose (GObject *object)
{
  GskGLLegacyRenderer *self = (GskGLLegacyRenderer *)object;

  if (self->driver != NULL)
    g_critical ("Attempt to dispose %s without calling gsk_renderer_unrealize()",
                G_OBJECT_TYPE_NAME (self));

  G_OBJECT_CLASS (gsk_gl_legacy_renderer_parent_class)->dispose (object);
}

static void
gsk_gl_legacy_renderer_class_init (GskGLLegacyRendererClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GskRendererClass *renderer_class = GSK_RENDERER_CLASS (klass);

  object_class->dispose = gsk_gl_legacy_renderer_dispose;

  renderer_class->supports_offload = TRUE;

  renderer_class->realize = gsk_gl_legacy_renderer_realize;
  renderer_class->unrealize = gsk_gl_legacy_renderer_unrealize;
  renderer_class->render = gsk_gl_legacy_renderer_render;
  renderer_class->render_texture = gsk_gl_legacy_renderer_render_texture;
}

static void
gsk_gl_legacy_renderer_init (GskGLLegacyRenderer *self)
{
}
...
gboolean
gsk_gl_legacy_renderer_try_compile_gl_shader (GskGLLegacyRenderer  *renderer,
                                       GskGLShader    *shader,
                                       GError        **error)
{
  GskGLProgram *program;

  g_return_val_if_fail (GSK_IS_GL_LEGACY_RENDERER (renderer), FALSE);
